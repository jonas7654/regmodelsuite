...
function. We are going to add all p predictors.

Here we use the pre calculated lambda grid with 10 lambdas with 5 folds for
cross validation.
```{r}
...
```
The "lasso_fit" object contains information about the cross validation process. 
To use the estimated lambda for the final model we can use \code{regmodel} again. 
It is also possible to extract scaled and unscaled coefficients from the model
by using coef().
