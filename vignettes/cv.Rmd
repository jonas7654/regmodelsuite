...
X6 <- runif(n, min = 0, max = 1)
X7 <- runif(n, min = 0, max = 1)
X8 <- runif(n, min = 0, max = 1)
X9 <- runif(n, min = 0, max = 1)
X10 <- runif(n, min = 0, max = 1)

# Add noise to y
y <- X1 + X2 + X3 + rnorm(n, mean = 0, sd = 0.3)

df <- data.frame(X1 = X1, X4 = X4, X5 = X5, X6 = X6, 
...
ridge_cv <- regmodel(formula = y ~ X1 + I(X1^3) + I(X1^5) + X4 + 
                       X5 + X6 + X7 + X8 + X9 + X10,
                data = df,
                model = "ridge",
                cv = TRUE,
                lambda = seq(0.001, 1, by = 0.001/100))
...
result_ridge$coefficients/result_ridge$sd_x
```
